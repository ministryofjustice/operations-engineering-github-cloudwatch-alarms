...
# main.tf
locals {
  # Splitting the events into smaller groups based on group_size
...
# CCloudWatch alarms for each metric
resource "aws_cloudwatch_metric_alarm" "unauthorised_users_modify_repository_settings_alarms" {
  count = length(local.event_groups)
